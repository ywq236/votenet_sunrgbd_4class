...
class VoteHead(BaseModule):
    ...
    def loss_by_feat(
        ...
        size_class_targets = size_class_targets.clamp(0, self.num_sizes - 1)
    ...
    def _get_targets_single(self,
        ...
        size_class_targets_clamped = size_class_targets.clamp(
            0, self.num_sizes - 1)
        if not torch.equal(size_class_targets, size_class_targets_clamped):
            import warnings
            warnings.warn(
                f'size_class_targets OOB: min={size_class_targets.min().item()}, '
                f'max={size_class_targets.max().item()}, '
                f'num_sizes={self.num_sizes}')
        one_hot_size_targets.scatter_(
            1, size_class_targets_clamped.unsqueeze(-1), 1)
        ...
        return (vote_targets, vote_target_masks, size_class_targets_clamped,
